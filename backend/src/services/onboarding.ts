...
const ABOUT_YOU: Screen = {
  ...
  fields: [
    ...
    { key: "start_weight_kg", label: "Current weight", type: "number", unit: "kg",
      min: 30, max: 300, optional: true,
      // The field most likely to make someone abandon onboarding, and the one
      // HealthKit can supply later. Never a blocker.
      placeholder: "Add your weight", hint: "Optional — I can pick this up from Health." },
    // Country and units are set in the nutrition profile; asking again would
    // tell the user the app is not paying attention.
...
const YOUR_DAY: Screen = {
  ...
  fields: [
    ...
    { key: "work_start", label: "Work or school starts", type: "time", optional: true },
    { key: "work_end", label: "…and ends", type: "time", optional: true },
    ...
    case "your_day":
      await q(
        `INSERT INTO daily_schedule
           (user_id, typical_bedtime, typical_wake_time, sleep_quality,
            activity_level, work_start, work_end)
         VALUES ($1, $2::time, $3::time, $4, $5, $6::time, $7::time)
         ON CONFLICT (user_id) DO UPDATE SET
           ...
           work_start        = COALESCE($6::time, daily_schedule.work_start),
           work_end          = COALESCE($7::time, daily_schedule.work_end),
           updated_at = now()`,
        [userId, answers.typical_bedtime ?? null, answers.typical_wake_time ?? null,
         answers.sleep_quality ?? null, answers.activity_level ?? null,
         answers.work_start ?? null, answers.work_end ?? null]);
